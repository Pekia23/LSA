...
// Nueva funcionalidad: Cuestionario RCM
function iniciarCuestionarioRCM() {
    document.addEventListener('DOMContentLoaded', function () {
        const nodos = {
            inicio: { pregunta: "¿Desea empezar el cuestionario de RCM?", si: "P1", no: "fin" },
            // Otros nodos aquí...
        };
        ...
        function mostrarPregunta(nodo) {
            const nodoObj = nodos[nodo];
            document.getElementById("questionText").textContent = nodoObj.pregunta;
            document.getElementById("yesBtn").style.display = nodoObj.si === null ? 'none' : 'inline-block';
            document.getElementById("noBtn").style.display = nodoObj.no === null ? 'none' : 'inline-block';
        }

        // Iniciar cuestionario cuando el modal se abre
        ...
        // Botones para avanzar en el cuestionario
        document.getElementById("yesBtn").addEventListener("click", function () {
            nodoActual = nodos[nodoActual].si;
            mostrarPregunta(nodoActual);
        });

        document.getElementById("noBtn").addEventListener("click", function () {
            nodoActual = nodos[nodoActual].no;
            mostrarPregunta(nodoActual);
        });

        document.getElementById('btn-siguiente').addEventListener('click', function() {
            window.location.href = 'registro_RCM_2';
        });
    });
}

// Llamada a la función del cuestionario
iniciarCuestionarioRCM();
